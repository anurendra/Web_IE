...
if __name__ == "__main__":
    ...
    parser.add_argument('--weights',  type=str, default='../models/snapshots/naive_test/snapshot_split_1_2500.caffemodel', help='Initialize with pretrained model weights')
    ...
    # print('o meri jaan, na ho pareshan')
    # print(screenshot_path)
    # print('bina tere mera sarna nahi')
    dom_path = os.path.join(download_dir,"dom.json")

    # LOAD POSITION LIKELIHOODS
    position_maps = load_position_maps(position_map_path)
    print('checking positon')
    print(len(position_maps))
    print(position_maps[0].shape)
    print(position_maps[1].shape)
    print(position_maps[2].shape)
    print(position_maps[3].shape)
    # print(position_maps[4].shape)
    print('bahut ho gya')

    # LOAD IMAGE BLOB
    im_blob = load_image_blob(screenshot_path)
    print('image shape is', im_blob.shape)
    ...
    print('Info about boxes')
    print(boxes_blob.shape)
    print(boxes_blob[:10])
    print('text and image blob shape')
    print(text_blob.shape)
    print(im_blob.shape)

  #  print('Now focus on leaf nodes')
  #  print('Total Number of leaves',len(leaf_nodes))
  #  print((leaf_nodes[0].keys()))
  #  print(leaf_nodes[0]['position'])
  #  print(leaf_nodes[0])

    # NET FORWARD
    net = net_forward(model, weights, im_blob, text_blob, boxes_blob)
 #   print(net['prob'].argmax())
